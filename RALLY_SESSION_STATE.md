...
**Última actualización:** v0.3.46 — 2026-07-16, sesión remota (Claude Code). Velocidad/fluidez visual del fondo Game of Life ajustable desde `/admin`. *Archivo optimizado el 2026-07-16 (mismo contenido sustancial, menos tokens).*
...
- **v0.3.46** — 🎛️ 2do parámetro admin del fondo Game of Life (pedido de Lucio: "que se vean más rápidos y fluidos los bits", no la velocidad de generación): `golBgFadeAlpha` (default 0.45, rango 0.05–1) — alpha del `destination-out` de la estela en `draw()`; mayor = las células muertas se apagan más rápido por frame (look más nítido/fluido), menor = más rastro/blur. Independiente de `golBgStepMs` (v0.3.45, cadencia de generaciones). Mismo patrón: `CFG` + `LAB_PARAMS` (game.js) + `PARAMS` (admin), sin tocar `database.rules.json`.
- **v0.3.45** — 🎛️ velocidad del fondo Game of Life ajustable desde `/admin` (pedido de Lucio): nueva clave `golBgStepMs` (default 140, ms por generación — menor = más rápido) sumada a `CFG`, `LAB_PARAMS` (game.js) y `PARAMS` (admin), mismo patrón que el resto del balance del juego — sin código nuevo de fontanería, ya viaja por `config/` (RTDB, lectura pública / escritura solo-admin) vía `applyRemoteConfig()`. `GolBg.tick()` ahora lee `CFG.golBgStepMs` en vez de un valor fijo; `this.stepMs` queda solo de fallback. Regla `database.rules.json` sin cambios (el nodo `config/$param` ya acepta cualquier clave alfabética + número, genérico).
- **v0.3.44** — 🧬 fix fondo Game of Life en PC (pedido de Lucio): el `<canvas id="gol-bg">` vivía DENTRO de `#screen-home`→`#app`, que tiene `max-width:520px` — en pantallas grandes el tablero quedaba recortado al ancho de "celular" y el resto del viewport se veía sin animar. Se movió a hijo directo de `<body>`, `position:fixed; inset:0; width:100vw; height:100dvh` (fuera de `#app`, que ahora es `z-index:1` para quedar arriba). Visibilidad ahora por clase `.is-shown` (toggleada en `GolBg.start()/stop()`) en vez de depender solo de `#screen-home.is-active` — evita que quede un frame congelado asomando detrás de otras pantallas. También, estético (pedido): celdas más chicas (`cell:14→9, gap:2→1.5`) y velocidad al doble (`stepMs:280→140`; umbral de reseed por estancamiento ajustado `90→180` pasos para mantener los mismos ~25s reales). `ResizeObserver` ahora observa `document.body` (antes `#screen-home`, que ya no contiene ni determina el tamaño del canvas). Verificado con Playwright viewport 1600px y 390px + transición a otra pantalla (sin canvas "fantasma").
